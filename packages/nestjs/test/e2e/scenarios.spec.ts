...
test.describe('@coraza/nestjs E2E', () => {
  test('1. benign request passes through (200)', async ({ request }) => {
    const res = await request.get('/')
    expect(res.status()).toBe(200)
    expect(await res.json()).toMatchObject({ ok: true, name: 'nestjs' })
  })
...
// Not in this E2E: custom onBlock override (unit-tested in test/guard.test.ts)
